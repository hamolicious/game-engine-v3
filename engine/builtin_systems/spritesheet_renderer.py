...
@system
def spritesheet_renderer(ecs: ECSManager) -> None:
    ...
    entities = list(
        ecs.find_entity_with_components(
            builtin_components.SpriteSheet, builtin_components.Transform2D
        )
    )
    ...
    for camera_id in cameras_ids:
        ...
        for entity_id in entities:
            ...
            sprite = cast(
                builtin_components.SpriteSheet, comps[builtin_components.SpriteSheet]
            )
            transform = cast(
        ...
        for render in renderables:
            ...
            camera.surf.blit(render[1].sheet[0][0], final_pos.xy)
